	...
	#generateAnimation(name, imageData, animationData)
	{
		...
		for(let idx = 0; idx < frames.length; idx++)
		{
			...
			if (!Array.isArray(frame[0]))
			{
				...
				// just the cols are given in shorthand format
				else
				{
					let cols = frame[1];
			...
			{
				// only the rows are in shorthand format
				if (!Array.isArray(frame[1]))
				{
					let rows = frame[0];
					for(let i = 0; i < rows.length; i++)
					{
						parsedFrames.push({ row: rows[i], col: frame[1]});
					}
				}
				// both the rows and the cols are in shorthand format
				else
				{
					let rows = frame[0];
					let cols = frame[1];
					for (let i = 0; i < rows.length; i++)
					{
						...
						{
							parsedFrames.push({ row: rows[i], col: cols[j]});
						}
					}
				}
			}
